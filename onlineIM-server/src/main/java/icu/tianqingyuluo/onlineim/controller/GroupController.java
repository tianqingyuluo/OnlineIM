...
public class GroupController {
     ...
     */
    @GetMapping("/{groupId}")
    public ResponseEntity<?> getGroupDetail(@RequestHeader("Authorization") String token, @PathVariable String groupId) {
        // TODO: 实现获取群组详情逻辑
        try {
            String userID = jwtUtil.getUserIDFromToken(token);
            GroupResponse response = groupService.getGroupDetailByID(groupId, userID);
            if (response == null) {
...
//         检查邀请人是否有权限邀请 (例如, 是否为群成员，且群设置允许成员邀请，或者邀请人是管理员/群主)
        ...
        try {
            for (String userID : userIdsToInvite) {
                boolean success = groupService.inviteUsersToGroup(groupId, userID, inviterId);
                if (success) {
